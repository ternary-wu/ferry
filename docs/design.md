...
### v1.1 UI 激进改造与项目层（2026-08-11）
- 按新设计文档（项目→工作空间→配置 + 未归类配置）引入 **Project 实体**：
  `IWorkspaceStore` 增加项目 CRUD；工作空间挂项目；配置可无工作空间（未归类）；
  新增 `config:move`（含跨工作空间/未归类移动）；`EnsureDefaultProject` 迁移历史无项目数据；
  存档包清单携带项目信息，导入按项目重建。
- 一级界面极简化：未打开配置时显示极简欢迎页（Logo + 一句说明 + 新建配置 + 最近使用插件）；
  打开配置后才进入编辑页，面包屑为"项目 / 工作空间 / 配置名称"。
- 侧栏：项目选择器（下拉含新建项目）、工作空间树（可折叠、悬停 ＋）、未归类配置组、
  左下角设置（插件管理/模板管理/系统设置/关于）。
- 配置菜单（⋯）：快速创建（同插件，跳过插件选择）/ 复制配置 / 移动工作空间 / 版本历史 / 导出存档 / 删除。
- 新建配置三步向导：搜索插件（含最近使用）→ 模板（默认模板兜底）→ 名称/工作空间（空 = 未归类）。
- 源码面板保持"默认隐藏、滑入 420px、可全屏"；状态记忆（当前项目/最近插件）存 localStorage。
- 自检扩至 19 步（含未归类创建、configs:unassigned、config:move），交互最差 37.4ms。

### UI 命名约定与停靠面板（2026-08-11）
- 命名约定（后续交流统一使用）：**侧边栏** = 左侧栏；**主工作面板** = 右侧工作区；
  **停靠面板** = 点击「源码」后停靠在主工作面板右侧的分区。
- 停靠面板当前只做"只读查看当前配置源码"：随表单编辑实时刷新渲染文本；
  宽度可在主工作面板占比 35%–60% 间拖动调整，缩到 35% 后再往右拖即关闭面板。
- 规划记录：停靠面板未来可能承载其他功能（编辑/导入导出等），暂不实现，保留扩展空间。
